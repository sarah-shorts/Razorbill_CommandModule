...
class RazorbillProgrammer (object):
    ...
    def __init__(self):
        ...
        # define a thermalization time here (seconds). This is the time the program waits after the dynacool is stable and before starting to measure.
        self.wait_t = 1
        ...
        self.SPARKY = self.rm.open_resource('ASRL4::INSTR') #Razorbill power supply RP100
    ...
    def monitor_determine_state(self):
        ...
        "Initial" should only ever be set at the beginning and flags the program to cool the ppms
        to the Tmin at the Trate. It should immediately be replaced by monitor_determine_state afterwards

        "Ready" indicates that the ppms is stable and has equilibrated at the Tmin... this indicates that
        we should begin a new strain value and restart a warming cycle
        ...
        "Finished" indicates that the ppms is sitting at Tmax and is stable, indicating that we need to cool
        back down to Tmin. The rest of the control is relinquished to the "Ready" state to set the next strain.
        ...
        tmax = self.guiDict.get("T_max").get("value")
        tmin = self.guiDict.get("T_min").get("value")
        tmax2 = self.guiDict.get("T_max2").get("value")
        tmin2 = self.guiDict.get("T_min2").get("value")
        mode = self.guiDict.get("d3TempExp").get("value")
        ttol = 0.1 #K
        ...
        if mode == "Ramp" or mode == "Hold":
            try:
                if self.monitorState == "Initial":
                    ...
                    self.monDict["measureStatus"].update({"value": "Ma'ii is in the INITIAL state. \n"
                                                                   ...
                                                                   "equilibration to T_min and H_field choice."})

                # if we have a stable temperature and we're within tolerance to t_min, we're in the Ready state.
                elif self.D3.temp_status == "Stable" and ((tmin+ttol) >= self.D3.temp >= (tmin-ttol)):
                    ...
                    time.sleep(self.wait_t)                 #Wait to thermalize before making a measurment
                    ...
                    self.monDict["measureStatus"].update({"value": "Ma'ii is in the READY state. \n"
                                                                   ...
                                                                   "and begin a new temperature ramp profile."})
                # if we have a stable temperature and we're within tolerance to t_max, we're in the Finished state.
                elif self.D3.temp_status == "Stable" and ((tmax+ttol) >= self.D3.temp >= (tmax-ttol)):
                    ...
                    self.monDict["measureStatus"].update({"value": "Ma'ii is in the FINISHED state. \n"
                                                                   ...
                                                                   "ready to begin cooling back to base temperature."})
                # if we have a stable temperature and we're within tolerance to t_max, we're in the Finished state.
                elif self.D3.temp_status in ["Stable", "Chasing", "Tracking", "Near"]:
            ...
            try:
                ...
                # if we have a stable temperature and we're within tolerance to t_min1, we're in the Ready1 state.
                elif self.D3.temp_status == "Stable" and ((tmin + ttol) >= self.D3.temp >= (tmin - ttol)):
                    ...
                    self.monDict["measureStatus"].update({"value": "Ma'ii is in the READY1 state. \n"
                                                                   ...
                                                                   "and begin a new temperature ramp profile."})
                # if we have a stable temperature and we're within tolerance to t_min1, we're in the Ready1 state.
                elif self.D3.temp_status == "Stable" and ((tmin2 + ttol) >= self.D3.temp >= (tmin2 - ttol)):
                ...
                # if we have a stable temperature and we're within tolerance to t_max, we're in the Finished state.
                    ...
                    self.monDict["measureStatus"].update({"value": "Ma'ii is in the FINISHED1 state. \n"
                                                                   ...
                                                                   "ready to begin cooling back to base temperature."})
                # if we have a stable temperature and we're within tolerance to t_max, we're in the Finished state.
                elif self.D3.temp_status == "Stable" and ((tmax2 + ttol) >= self.D3.temp >= (tmax2 - ttol)):
    ...
    def monitor_control(self):
        ...
        tmax = self.guiDict.get("T_max").get("value")
        tmin = self.guiDict.get("T_min").get("value")
        trate = self.guiDict.get("T_rate").get("value")
        hfield = self.guiDict.get("H_field").get("value")
        tmax2 = self.guiDict.get("T_max2").get("value")
        tmin2 = self.guiDict.get("T_min2").get("value")
        trate2 = self.guiDict.get("T_rate2").get("value")
        mode = self.guiDict.get("d3TempExp").get("value")
        ...
        if mode == "Ramp":
            ...
            # check if we're in the ready state
                ...
                print('MultiVu command... go to {field}Oe at 100Oe/min'.format(field=hfield))
                self.D3.set.field(hfield, 100, 0, 1)
                ...
                # set temperature using T_max and T_rate
                print('MultiVu command... go to {temp}K at {rate}K/min'.format(temp=tmax, rate=trate))
                self.D3.set.temp(tmax, trate, 0)
            ...
            # check if we're in the ready state FOR REGIME 1
                ...
                # set temperature using T_max and T_rate
                print('MultiVu command... go to {temp}K at {rate}K/min (regime 2)'.format(temp=tmax2, rate=trate2))
                self.D3.set.temp(tmax2, trate2, 0)
                self.isMeasuring = True
            ...
            # check if we're in the finished state
                ...
                print("the measurement has completed and we're ready to cool again to Tmin1")
    ...
    def setup_temp_compliance(self):
        ...
        max_temp = float(self.guiDict.get("T_max").get("value"))
        min_temp = float(self.guiDict.get("T_min").get("value"))
        max_temp2 = float(self.guiDict.get("T_max2").get("value"))
        min_temp2 = float(self.guiDict.get("T_min2").get("value"))

        # update the built in bounds using dictionary methods
        self.guiDict["T_max"].update({"bounds": [300.0, min_temp]})
        self.guiDict["T_min"].update({"bounds": [max_temp, 1.8]})
